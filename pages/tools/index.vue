<template>
    <main class="flex pt-[var(--header-bottom)] h-[calc(100vh-var(--header-height)-5rem)]">
        <aside class="fixed inline-block min-h-full text-d-base px-10 border-r border-light-100/10">
            <ul class="w-[15rem]">
            ...
            <div class="mt-10">
                <UButton size="sm" color="white">Filter</UButton>
            </div>
        </aside>
        ...
        <section class="w-full ml-[calc(15rem+(2*2.5rem))] pl-10">
            ...
            <div class="flex justify-between items-center pb-5 mr-10">
                <p class="font-light text-d-lg flex gap-3 items-end">
                    <span v-if="search === ''" class="font-extrabold text-d-2xl leading-tight">{{ allTools }}</span>
...
const { getPageTools, getToolsByTitle } = useTools();
const tools = ref();
...
async function Search() {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        try {
            tools.value = await getToolsByTitle(search.value);
            return
        } catch (error) {
            return
        }
...
const getMoreTools = computed(() => {
    ...
    if (allTools.value === tools.value.length || search.value !== '') {
